...
if st.session_state.get('active_program') == 'SBI':
    ...
    st.stop()  # Prevent the MR SIA dashboard from executing in SBI mode
...
try:
    ...
    with tab_total:
            ...
            if exec_target_mode in ["Projected Population Target", "Actual RHU Target"]:
                ...
                if view_mode == "All Municipalities (Abra)":
                    ...
                    if abra_geo and not df_geo_summary.empty:
                        ...
                        if 'Vit A Coverage %' in df_geo_summary.columns:
                            fig_map_va = px.choropleth_map(
                                df_geo_summary, geojson=abra_geo, locations='Map_Location', featureidkey="properties.Standard_Name", 
                                color='Vit A Coverage %', color_continuous_scale="RdYlGn", range_color=[0, 100], map_style="white-bg",
                                zoom=9.2, center={"lat": 17.58, "lon": 120.80}, opacity=0.7, hover_name=geo_col,
                    ...
                    if brgy_geo and not df_geo_summary.empty:
                        ...
                        # ==================================
                        # 1 COLUMN BARANGAY MAP LAYOUT
                        # ==================================
                        st.markdown("**Measles-Rubella (MR) Coverage**")
                        
                        fig_map_brgy_mr = px.choropleth_map(
        ...
        if view_mode == "All Municipalities (Abra)":
            ...
            with st.expander("📥 View & Download Municipal OPT Report", expanded=False):
                ...
                # --- THE FIX: Locked in exactly 5 columns, dropping the diff ---
                df_muni_report = df_muni_report[['Municipality', '6-59m OPT', 'Actual Target', 'MR Vaccinated', 'Actual Coverage']]
                ...
                fig_opt_table = go.Figure(data=[go.Table(
                    ...
                    cells=dict(
                        ...
                        font=dict(color='#1E293B', size=12),
                        # Adjusted to exactly 5 alignments
    ...
    with tab_vt_lgu:
        ...
        # ==========================================
            ...
            st.write(f"Detailed breakdown of doses administered and target coverage per barangay in {selected_car_prov}.")
